...
public class ShahedDroneUnit : MonoBehaviour
{
    ...
    public float impactDistance = 1.2f;
    public GameObject explosionEffectPrefab;

    [Header("Visual Body")]
    public Transform bodyTransform;

    private ShahedState _state = ShahedState.Idle;
    ...
    void Update()
    {
        switch (_state)
        {
            case ShahedState.Idle: UpdateIdle(); break;
            case ShahedState.Move: UpdateMove(); break;
            case ShahedState.Attack: UpdateAttack(); break;
        }
    }
    ...
    void ChangeState(ShahedState newState)
    {
        _state = newState;
        if (newState != ShahedState.Move)
        {
            _path = null;
            _reachedEnd = false;
        }
    ...
    void OnPathComplete(Path p)
    {
        if (p.error)
        {
            Debug.LogWarning($"{name}: A* path error -> {p.errorLog}");
            ChangeState(ShahedState.Idle);
            return;
        }
